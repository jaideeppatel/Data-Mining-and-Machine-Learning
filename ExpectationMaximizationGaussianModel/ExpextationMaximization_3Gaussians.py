...
def Expectation_Maximization(i):
    ...
    # print(data_sq,data_sq.shape,data_norm.shape)
    for c in range(g):
    ...
    for c in range(g):
        ...
        for items in clusters[c]:
            b = Gaussian_func(items, new_params[c][0], new_params[c][1])
            a = math.log(Gaussian_func(items, new_params[c][0], new_params[c][1]),10)
            # print(b)
            total = total + a
        LL[0][c] = total
...
if __name__ == '__main__':
    ...
    for i in distributions:
        ...
        plt.savefig('LL for Data1 for Gaussian - 3.png')